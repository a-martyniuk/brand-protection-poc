...
const ProductListView: React.FC<ProductListViewProps> = ({ products, loading }) => {
    ...
    const [sortBy, setSortBy] = useState<'fraud_score' | 'price' | 'match_level'>('fraud_score');
    ...
    const toggleSort = (field: 'fraud_score' | 'price' | 'match_level') => {
        if (sortBy === field) {
    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {/* Table Header */}
                    <div className="hidden md:grid grid-cols-[80px_1fr_200px_120px_100px_100px_120px] gap-4 px-4 py-2 text-[10px] uppercase tracking-wider font-black text-slate-500">
                        <div></div>
                        <div>Product</div>
                        <div>Seller</div>
                        <div
                    ...
                    {sortedProducts.map(product => (
                        <div
                            ...
                            className="grid grid-cols-1 md:grid-cols-[80px_1fr_200px_120px_100px_100px_120px] gap-4 items-center p-4 bg-slate-900/40 hover:bg-slate-900/60 border border-white/5 hover:border-brand-500/30 rounded-2xl cursor-pointer transition-all group"
                        >
                            {/* Thumbnail */}
